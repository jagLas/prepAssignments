...
function doubler(nums) {
    ...
    // Your code here
    //Write a loop that starts at 0 array value and doubles. Moving up by one until length is met.
    for (let step=0; step < (nums.length); step++){
        nums[step] = nums[step]*2;}
     return nums;
};
...
function fizzBuzz(max) {
    ...
    let fizzedArray = []
    for (let i=0; i < max; i++){
        //check if divisible by both, and do not add to array
        if (i % 3 === 0 && i % 5 === 0){
        }
        //check if neither divisble by 3 and 5
            else if (i % 3 > 0 && i % 5 > 0){
        }
        //add to new array
            else{
...
function isSubString(searchString, subString) {
    ...
    //Change cases to match
    searchString = searchString.toLowerCase()
    subString = subString.toLowerCase()

    //Write if function to evaluate of substring
